...
- [x] 4.2 Locate the "What would happen without this charity?" section. Change the template variable from `.description` to `.source.quote`.
- [x] 4.3 Inject the `provenance-badge.html` partial next to the quote, passing in the `.source.url` and `.source.quote` to generate the W3C Text Fragment link.
- [x] 4.4 Open `web/layouts/partials/audit-checklist.html`.
- [x] 4.5 Delete the hardcoded `$tooltips` Hugo dictionary and remove the `title` attributes from the checklist rows.
- [x] 4.6 Inside the `<details>` expansion block, completely restructure the HTML.
