...
- [ ] 2.2 In `utils_api/app/audits/impact.py`, locate the `calculate_ies` function.
- [ ] 2.3 Inside the `for metric in record.impact.metrics.metrics:` loop, update the `ies_breakdown.append(...)` dictionary.
- [ ] 2.4 Add `"source": metric.source.model_dump(exclude_unset=True) if metric.source else None` to the appended dictionary.
