...
- [x] 2.1 Insert a Code Node immediately after "Parsed Content (Financials)".
...
- [x] 2.2 Insert an HTTP Request Node to call `https://api.frankfurter.dev/v1/{{ $json.targetDate }}?base={{ $json.original_code }}&symbols=USD`.
- [x] 2.3 Insert a Code Node to map the response back into the main payload, setting `usd_exchange_rate = response.rates.USD` and `rate_date = response.date`.
