...
- [ ] 2.1 Refactor `app/schemas/analytics.py` to reflect the schema changes (create the `CalculatedMetric` model).
- [ ] 2.2 Modify `app/audits/impact.py`:
...
- [ ] 2.3 Modify `app/audits/financial.py`:
  - Update `check_reserve_cap`: `pass` <= 2, `warning` > 2 and <= 5, `fail` > 5.
  - Update `check_liquidity`: `pass` >= 6, `warning` >= 3 and < 6, `fail` < 3.
- [ ] 2.4 Update `app/routers/audit.py` to route boolean checks to `check_items` and quantitative metrics to `calculated_metrics`.
- [ ] 2.5 Update `tests/test_audit_financial.py` and `tests/test_audit_impact.py` to reflect the new three-tier boundaries and the new payload structure.
