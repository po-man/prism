...
- [x] 2.3 Update `generate_extraction_schemas.py` to handle the generation of these split schemas from the master `impact.schema.json`.
...
- [x] 4.1 Run an end-to-end audit on a highly complex organisation (e.g., multi-domain with large annual reports) to ensure the parallel extraction successfully merges and passes the strict `/validate` endpoint.
- [x] 4.2 Verify cost/token reduction by comparing the new n8n execution logs against previous executions.