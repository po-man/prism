...
1.  **Context Hierarchy and Provenance**: You will be provided with official PDF text. You must extract all information exclusively from the provided document(s).
    ...
    -   If the charity explicitly states the exact cost to help *one individual animal* (e.g., 'It costs $25 to spay a dog', 'Sponsor a farm rescue for £50'), you must capture each statement as an entry in the `explicit_unit_costs` array. For each entry:
      ...
      Do NOT extract aggregate budgets, total grant awards, monthly team costs, or overall fundraising campaign targets. It MUST be a discrete unit cost per individual animal. Do NOT attempt to calculate unit costs yourself; only record explicitly stated costs. If no explicit unit costs are found, set `explicit_unit_costs` to an empty array.
